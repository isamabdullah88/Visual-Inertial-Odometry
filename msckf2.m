function msckf2(imuNoise, imuBias, camNoise)
...
% ################## LOADING IMAGE DATA ###################################
%%%% MAV ETH DATASET %%%%%
fileDir = 'Data\2011_09_26_drive_0095_sync\image_00\data';
...
[noiseParams, camParams] = setNoiseParams(C_c_v, rho_v_c_v, imuNoise, imuBias, camNoise);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if (mod(State.N, Parameters.NMAX_FRAMES) == 0)
            if (countImg > 0)
                ...
                [State, costVec] = updateStateAndCov(State, noiseParams, camParams, costVec);
                [State, Parameters] = startTracking( currImg, pointTracker, State, camParams, Parameters);
            ...
            [ State, currTrackedPts, costVec2 ] = handleTrackedFeatures(pointTracker, ...
                currImg, State, camParams, costVec2, noiseParams);
...
figure; line(statePos(:,1), statePos(:,2), statePos(:,3),'Color', 'g');
...
% %
%%% GPS Plot
tGPS = toGlobalCoords2(gpsReadings);
% tGPS = gpsReadings;
