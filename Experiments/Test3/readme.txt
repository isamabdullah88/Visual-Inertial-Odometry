This result is obtained using fixed imu parameters and varing camera parameters. Specifically the
number of camera States tracked has been increased, while the number of features to extract have
been kept fixed.
Also, the in-between update has been disabled.
Detect Threshold has been disabled.

No reasonable result has been obtained.

Following are the values:

camStates: 15
no of features detected: 30 (corner)


imuNoise : 1.08e-4;
imuBias : 1.00e-5;

camNoise:
    0.1000
    0.2000
    0.3000
    0.4000
    0.5000
    0.6000
    0.7000
    0.8000
    0.9000
    1.0000
    1.1000
    1.2000
    1.3000
    1.4000
    1.5000
    1.6000
    1.7000
    1.8000
    1.9000
    2.0000
    2.1000
    2.2000
    2.3000
    2.4000
    2.5000
    2.6000
    2.7000
    2.8000
    2.9000
    3.0000
    3.1000
    3.2000
    3.3000
    3.4000
    3.5000
    3.6000
    3.7000
    3.8000
    3.9000
    4.0000
    4.1000
    4.2000
    4.3000
    4.4000
    4.5000
    4.6000
    4.7000
    4.8000
    4.9000
    5.0000
